  ...
  return (
    <header className="header">
      <nav className="navbar container px-1 sm:px-8">
        ...
        <div className="xl:ml-16 text-xl font-bold"><span className="text-primary">تفاح </span><span className="text-secondary dark:text-red-500">شامي</span></div>
                ...
                <React.Fragment key={`menu-${i}`}>
                  {menu.hasChildren ? (
                    <li className="nav-item nav-dropdown group relative">
                      <span className={`nav-link ${menu.children.map((c) => c.url).includes(router.asPath) && "active"} inline-flex items-center`}>
                        {menu.name}
                        <svg className="h-4 w-4 fill-current" viewBox="0 0 20 20">
      ...
      {showMenu && <div className="header-backdrop absolute left-0 top-0 h-[100vh] w-full bg-black/50 lg:hidden"></div>}
    </header>
  );
};
